...
func Initialize(ctx context.Context) *provider.Services {
	...
	emoticonService := service.NewEmoticonService(emoticonDao)
	filesystemFilesystem := filesystem.NewFilesystem(config)
	emoticon := v1.NewEmoticonHandler(emoticonService, filesystemFilesystem)
