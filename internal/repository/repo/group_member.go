...
// IsMember 检测是属于群成员
func (repo *GroupMember) IsMember(gid, uid int, cache bool) bool {
	if cache && repo.relation.IsGroupRelation(context.Background(), uid, gid) == nil {
		return true
	}
...
// GetMembers 获取群组成员列表
func (repo *GroupMember) GetMembers(groupId int) []*model.MemberItem {
	fields := []string{
		"group_member.id",
		"group_member.leader",
