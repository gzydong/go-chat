...
func (u *User) Detail(ctx *gin.Context) error {
	...
	if err != nil {
		return ichat.SystemError(ctx, err.Error())
	}

	return ichat.Success(ctx, &web.GetUserInfoResponse{
		Id:       user.Id,
		Mobile:   user.Mobile,
...
func (u *User) ChangeDetail(ctx *gin.Context) error {
	...
	return ichat.Success(ctx, nil, "个人信息修改成功！")
}
...
func (u *User) ChangePassword(ctx *gin.Context) error {
	...
	if uid == 2054 || uid == 2055 {
		return ichat.BusinessError(ctx, "预览账号不支持修改密码！")
	}

	if err := u.service.UpdatePassword(jwtutil.GetUid(ctx), params.OldPassword, params.NewPassword); err != nil {
		return ichat.BusinessError(ctx, "密码修改失败！")
	}

	return ichat.Success(ctx, nil, "密码修改成功！")
}
